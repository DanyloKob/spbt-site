/**
 * Canvas and pointer effects: background lattice, hero mark, custom cursor.
...
/* ─── Hero mark ──────────────────────────────────────────────────────────
   The house mark as a solid slab of dots, rotated and projected by hand.

   The shape is not hand-plotted twice: the same polygon that the favicon and
   the navbar use is filled once into an offscreen canvas, its pixels are read,
   and every filled sample becomes a dot. One source of truth for the emblem —
   edit MARK and the icon, the favicon and this object stay the same thing.
   Each dot gets a random z inside a slab, which is what gives the object
   ...
   (The portfolio runs the same machinery on a `</>` glyph sampled from
   Unbounded. Here the subject is the brand mark, so there is no font to wait
   for — which is the only structural difference between the two.)         */
const MARK = [
  [1000, 510], [1420, 755], [1420, 1000], [1108, 818], [1108, 1364],
  [892, 1490], [892, 818], [580, 1000], [580, 755],
];
const MARK_BOX = { x: 580, y: 510, w: 840, h: 980 };

/** Read the mark's filled pixels into points centred on the origin. */
function sampleMark(w, h, step, pad) {
  const off = document.createElement('canvas');
  off.width = Math.max(1, Math.round(w));
  off.height = Math.max(1, Math.round(h));
  const g = off.getContext('2d');

  // Fit the polygon's real bounding box inside the padded canvas, centred.
  const s = Math.min(
    (off.width - pad * 2) / MARK_BOX.w,
    (off.height - pad * 2) / MARK_BOX.h,
  );
  g.translate(off.width / 2, off.height / 2);
  g.scale(s, s);
  g.translate(-(MARK_BOX.x + MARK_BOX.w / 2), -(MARK_BOX.y + MARK_BOX.h / 2));

  g.beginPath();
  MARK.forEach(([x, y], i) => (i ? g.lineTo(x, y) : g.moveTo(x, y)));
  g.closePath();
  g.fillStyle = '#fff';
  g.fill();

  // getImageData reads device pixels, so the transform above does not apply.
...
export function initHeroMark() {
  const canvas = document.getElementById('hero-mark');
  if (!canvas) return;
  ...
  /* nx/ny are the pointer in -1..1 across the canvas. The object reads them
     ...
     mark floats without a card around it. */
  const pointer = { x: 0, y: 0, nx: 0, ny: 0, on: false };

  function size() {
    ({ ctx, w, h } = fit(canvas));
    /* Denser on roomy cards, sparser on small ones — the dot count drives cost.
       Wider steps than the portfolio's glyph on purpose: the mark's ink covers
       ~42% of its bounding box against a slim `</>`, so an identical step gave
       three times the dots for no visible gain. */
    const step = w < 320 ? 3 : 4;
    pts = sampleMark(w, h, step, Math.max(18, w * 0.09)).map(([x, y]) => ({
      bx: x,
      by: y,
  ...
  /* Same reasoning as the lattice: on a touch screen there is no hover, so a
     tap would latch the rotation and leave it stuck. The mark keeps its idle
  ...
  const start = () => {
    ...
    /* The mark is one screenful of a seven-screen page. `loop()` only pauses
       when the tab is hidden, so without this it keeps sorting and painting
       ~1500 dots every frame while the canvas sits thousands of pixels above
  ...
  /* Nothing to wait for: the shape comes from a polygon, not a typeface. */
  start();
}
