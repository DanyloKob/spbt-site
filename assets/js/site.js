/**
 * Navigation, slide scrolling, page transitions, and the entry point that boots
 * every effect. Loaded by every page as a single <script type="module">.
 */

import { initBgGrid, initHeroMark, initCursor } from './fx.js';
import { initReveal, initTilt, initMagnetic, initCounters, initTypeIn } from './motion.js';

const reduced = () => matchMedia('(prefers-reduced-motion: reduce)').matches;

/* ─── Navbar: solid on scroll, hidden on scroll-down ───────────────────
   The bar carries a wordmark and one action, so there is no active-link
   indicator to drive. The `[data-nav-link]` half below stays because it costs
   nothing and works the moment a section link is added to the bar.        */
...
export function initSlideWheel() {
  ...
  addEventListener('wheel', (e) => {
    ...
    /* Normally two neighbouring stops are exactly one viewport apart, so one
       gesture is one slide. But in a window too small for a slide to fit — a
       narrow desktop stacks the hero, which then runs several hundred pixels
       past the screen — the gap is larger, and jumping the whole way would skip
       what is in between: content still reachable by keyboard and scrollbar, but
       never by the wheel. There, move most of a screen and let the next gesture
       carry on; the shorter lock keeps that from feeling sluggish. */
    const far = Math.abs(next - here) > innerHeight * 1.15;
    const target = far ? here + dir * Math.round(innerHeight * 0.85) : next;

    e.preventDefault();
    lockedUntil = now + (far ? 380 : 620);
...
function boot() {
  ...
  const modules = [
    initBgGrid, initHeroMark, initCursor,
